...
namespace SIGVerse.SampleScenes.Hsr.HsrCleanupVR
{
	...
	public class CleanupAvatarVRHandControllerForRift : MonoBehaviour, IPunObservable
	{
		...
		public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
		{
			if (stream.IsWriting)
			{
				this.handPostureRatio = (this.handType == HandType.LeftHand) ? OVRInput.Get(OVRInput.RawAxis1D.LHandTrigger) : OVRInput.Get(OVRInput.RawAxis1D.RHandTrigger);
		...
		void LateUpdate()
		{
			float ratio = (this.photonView.IsMine)? (this.handType == HandType.LeftHand) ? OVRInput.Get(OVRInput.RawAxis1D.LHandTrigger) : OVRInput.Get(OVRInput.RawAxis1D.RHandTrigger) : this.handPostureRatio;
